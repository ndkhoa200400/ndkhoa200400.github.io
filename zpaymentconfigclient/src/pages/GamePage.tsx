import { Breadcrumb, Button, Card, Col, Form, Image, Input, Row } from "antd";
import { useEffect, useState } from "react";
import { Link, useNavigate, useParams } from "react-router-dom";
import { LoadingFullScreen } from "../components/LoadingFullScreen";
import { IGame } from "../types/type";
import {
  callAPI,
  CallAPIOptionsDefault,
  getGameById,
  updateGame,
} from "../utils/api-hepler";

export const GamePage = () => {
  let params = useParams();
  const appId = params.appId;
  const navigate = useNavigate();

  const [game, setGame] = useState<IGame>();
  const [form] = Form.useForm<IGame>();
  const onFinish = async (values: IGame) => {
    callAPI(
      updateGame,
      [Number(appId), values],
      setGame,
      CallAPIOptionsDefault
    );
  };

  useEffect(() => {
    if (appId) {
      callAPI(getGameById, [Number(appId)], setGame, {
        onError: () => {
          navigate("./../");
        },
      });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [appId]);

  return game ? (
    <div>
      <Breadcrumb className="mb-3">
        <Breadcrumb.Item>
          <Link to="/games">Danh sách game</Link>
        </Breadcrumb.Item>
        <Breadcrumb.Item>
          <div>Game</div>
        </Breadcrumb.Item>
      </Breadcrumb>
      <Card
        title="Cài đặt"
        headStyle={{
          background: "#f7fafc",
          fontSize: "1.2rem",
          textAlign: "left",
          fontWeight: "bold",
        }}
      >
        <Form
          layout="vertical"
          form={form}
          onFinish={onFinish}
          initialValues={game}
          size="middle"
        >
          <Row gutter={{ xs: 24, sm: 16, md: 24, lg: 32 }}>
            <Col xs={24} lg={12} className="gutter-row">
              <Form.Item label="ID ứng dụng" name="appId">
                <Input disabled value={game.appId} />
              </Form.Item>
            </Col>

            <Col xs={24} lg={12} className="gutter-row">
              <Form.Item label="Tên hiển thị" name="name">
                <Input />
              </Form.Item>
            </Col>
          </Row>
          <Row gutter={{ xs: 24, sm: 16, md: 24, lg: 32 }}>
            <Col xs={24} lg={12} className=" mt-4 gutter-row">
              <Form.Item label="Link Logo" name="logo">
                <Input
                  onChange={(e) => {
                    setGame({
                      ...game,
                      logo: e.target.value,
                    });
                  }}
                />
              </Form.Item>
              <div
                style={{
                  textAlign: "left",
                }}
              >
                <Image src={game.logo} height="128px"></Image>
              </div>
            </Col>

            <Col xs={24} lg={12} className="gutter-row mt-4">
              <Form.Item label="Link Background" name="background">
                <Input
                  onChange={(e) => {
                    setGame({
                      ...game,
                      background: e.target.value,
                    });
                  }}
                />
              </Form.Item>
              <div
                style={{
                  textAlign: "left",
                }}
              >
                <Image src={game.background} height="128px"></Image>
              </div>
            </Col>
          </Row>

          <Row className="mt-4" gutter={{ xs: 24, sm: 16, md: 24, lg: 32 }}>
            <Col xs={24} lg={12} className="gutter-row mt-4">
              <Form.Item label="Link Banner 1" name="banner1">
                <Input
                  onChange={(e) => {
                    setGame({
                      ...game,
                      banner1: e.target.value,
                    });
                  }}
                />
              </Form.Item>
              <div
                style={{
                  textAlign: "left",
                }}
              >
                <Image src={game.banner1} height="128px"></Image>
              </div>
            </Col>

            <Col xs={24} lg={12} className="gutter-row mt-4">
              <Form.Item label="Link Banner 2" name="banner2">
                <Input
                  onChange={(e) => {
                    setGame({
                      ...game,
                      banner2: e.target.value,
                    });
                  }}
                />
              </Form.Item>
              <div
                style={{
                  textAlign: "left",
                }}
              >
                <Image src={game.banner2} height="128px"></Image>
              </div>
            </Col>
          </Row>

          <Form.Item className="mt-6">
            <Button type="primary" htmlType="submit">
              Lưu
            </Button>
          </Form.Item>
        </Form>
      </Card>
    </div>
  ) : (
    <LoadingFullScreen />
  );
};
